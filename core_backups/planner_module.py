...
class Planner:
    ...
    def __init__(self,
                 query_llm_func: Callable[[str, Optional[str], bool, int], str],
                 prompt_manager: Optional[PromptManager],    # No quotes needed
                 goal_monitor: Optional[GoalMonitor],        # No quotes needed
                 mission_manager: Optional[MissionManager],  # No quotes needed
                 suggestion_engine: Optional[SuggestionEngine], # No quotes needed
                 config: Dict[str, Any],
                 logger=None,
    ...
    def _construct_prompt(self, context: Dict[str, Any]) -> str:
        if not self.prompt_manager: return "Error: PromptManager not available for Planner."
        
        base_prompt_template_name = "planner_decide_action" 
        template_fill_data = context.copy() 
        if isinstance(template_fill_data.get("active_goals"), list):
            template_fill_data["active_goals"] = ", ".join(template_fill_data["active_goals"]) if template_fill_data["active_goals"] else "None"
        
        if isinstance(template_fill_data.get("conversation_history"), list):
            formatted_history = []
            for turn in template_fill_data["conversation_history"]:
                speaker = turn.get("role", turn.get("speaker", "Unknown")) 
                content = turn.get("content", turn.get("text", ""))
                formatted_history.append(f"{speaker.capitalize()}: {content}")
            template_fill_data["conversation_history_str"] = "\n".join(formatted_history) 
        else:
             template_fill_data["conversation_history_str"] = "(No conversation history provided to template)"

        # Add general config to template data if prompts need it
        template_fill_data["config"] = self.config 

        prompt = self.prompt_manager.get_filled_template(base_prompt_template_name, template_fill_data)

        if context.get("pending_suggestions"):
            formatted_suggestions_for_prompt = []
            for sugg in context["pending_suggestions"]: # sugg is the loop variable
                s_id = sugg.get('id','N/A')[-6:] if sugg.get('id') else 'N/A'
                desc_sugg = sugg.get('description', sugg.get('suggestion', 'N/A'))
                formatted_suggestions_for_prompt.append(
                    f"  - ID: ..{s_id}, Desc: \"{desc_sugg[:80]}...\", Prio: {sugg.get('priority','N/A')}, Origin: {sugg.get('origin','N/A')}" # Corrected from s_ugg to sugg
                )

            suggestion_management_instructions = self.prompt_manager.get_filled_template(
                "planner_manage_suggestions_instructions",
                {"pending_suggestions_formatted_list": "\n".join(formatted_suggestions_for_prompt) if formatted_suggestions_for_prompt else "(None to display)"}
            )
            prompt += f"\n\n{suggestion_management_instructions}"
        
        prompt += "\n\nBased on all the above, what is the best next_action and its details? Respond ONLY with a valid JSON object representing your decision."
        return prompt
