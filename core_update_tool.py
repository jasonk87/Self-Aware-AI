...
def apply_update(target_file: str, approved_by: str = "AI"):
    ...
    if not os.path.exists(target_file_abs):
        ...
        logger.info(f"Target file {target_file_abs} does not exist. Applying as new file.")
        ...
        logger.info(f"✅ Core file {os.path.basename(target_file_abs)} created (version {ver} - placeholder)") # Adjust versioning as needed
        return
    ...
    logger.info(f"Backed up {target_file_abs} to {backup_dest_path}")
    
    try:
        shutil.move(new_path, target_file_abs)
    except Exception as e:
        logger.error(f"❌ Error moving {new_path} to {target_file_abs}: {e}")
    ...
    logger.info(f"✅ Core file {os.path.basename(target_file_abs)} updated (version {ver} backed up)")
...
def rollback_version(version_to_restore: str) -> bool:
    ...
    if not items_for_version:
        logger.warning(f"No backups found for version '{version_to_restore}'."); return False
    ...
    if not files_to_restore_map:
        logger.warning(f"No valid entries with filenames found for version '{version_to_restore}'."); return False
    ...
    for original_filename, entry_to_restore in files_to_restore_map.items():
        ...
        if not os.path.exists(backup_path):
            logger.error(f"Backup file {backup_path} for {original_filename} (version {version_to_restore}) not found. Skipping.")
            continue
        ...
        try:
            ...
            logger.info(f"Restored {target_path} from backup {os.path.basename(backup_path)}")
    ...
    if restored_files_count > 0:
        ...
        logger.warning(f"No files were actually restored for version '{version_to_restore}'. Check backup paths and file existence.")
        return False
...
if __name__ == "__main__":
    if len(sys.argv) < 2: # Adjusted for single command like "test" without file
        logger.info("Usage:")
        logger.info("  python core_update_tool.py test_syntax <FILE_new.py>")
        logger.info("  python core_update_tool.py test_suite")
        logger.info("  python core_update_tool.py stage <FILE_to_update.py> <NEW_CODE_FILE.py> [approver]")
        logger.info("  python core_update_tool.py apply <FILE_to_update.py> [approver]")
        logger.info("  python core_update_tool.py rollback <VERSION>")
        sys.exit(0)
    ...
    if cmd == "test_syntax":
        if len(sys.argv) < 3:
            logger.info("Usage: python core_update_tool.py test_syntax <FILE_new.py>")
            sys.exit(1)
        ...
        if len(sys.argv) < 4:
            logger.info("Usage: python core_update_tool.py stage <FILE_to_update.py> <NEW_CODE_FILE.py> [approver]")
            sys.exit(1)
        ...
        if not os.path.exists(new_code_file_path):
            logger.error(f"Error: New code file not found: {new_code_file_path}")
            sys.exit(1)
        
        with open(new_code_file_path, 'r', encoding='utf-8') as f_new_code:
            new_code_content = f_new_code.read()
            
        logger.info(f"Staging update for '{target_py_file}' with code from '{new_code_file_path}'...")
        ok, msg = update_core_file(new_code_content, target_py_file) # approver is for apply_update
        if ok: # "✅"
            logger.info(msg)
            logger.info(f"Staging successful. To apply, run: python core_update_tool.py apply {target_py_file} [approver]")
        else: # "❌"
            logger.error(msg)
        sys.exit(0 if ok else 1)
        ...
        if len(sys.argv) < 3:
            logger.info("Usage: python core_update_tool.py apply <FILE.py> [approver]")
            sys.exit(1)
        tgt = sys.argv[2]
        ...
        try:
            ...
            logger.error("Ensure you have staged the update first using 'stage' command or that _new.py file exists.")
            sys.exit(1)
        except Exception as e_apply:
            logger.error(f"An error occurred during apply: {e_apply}")
            sys.exit(1)
        sys.exit(0)
            
    elif cmd == "rollback":
        if len(sys.argv) < 3:
            logger.info("Usage: python core_update_tool.py rollback <VERSION>")
            sys.exit(1)
        ver = sys.argv[2]
