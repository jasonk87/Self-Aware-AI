...
# Attempt to import necessary modules with fallbacks for console robustness
try:
...
# For colors (optional dependency)
try:
...
META_DIR = "meta"
GOAL_FILE_CONSOLE = os.path.join(META_DIR, "goals.json") 
CHANGELOG_FILE_CONSOLE = os.path.join(META_DIR, "changelog.json")
# LAST_UPDATE_FLAG_CONSOLE is now NOTIFIER_LAST_UPDATE_FLAG from notifier_module
ACTIVE_GOAL_FILE_CONSOLE = os.path.join(META_DIR, "active_goal.json") 
TOOL_REGISTRY_CONSOLE_FILE = os.path.join(META_DIR, "tool_registry.json") 
LAST_REFLECTION_CONSOLE_FILE = os.path.join(META_DIR, "last_reflection.json") # For /status, from prompt_manager

partial_input: str = "" 
active_ai_streams = {} # Key: stream_id, Value: {"prefix_printed": bool, "accumulated_text": str, "original_query": str, "thread_id_for_stream": str}
...
last_ai_interaction_thread_id: str | None = None # Stores the thread_id of the last AI response
...
MAX_SNIPPET_TEXT_LEN = 250 # Max characters per turn text in the snippet
...
def get_formatted_history(thread_id: str) -> str:
    ...
    for turn in turns_to_include:
        ...
        text = turn.get("text", "")
        # Simple formatting; LLMs are good with "Speaker: Text"
...
def print_and_restore_prompt(message_to_print: str, is_stream_chunk: bool = False, stream_id: str | None = None, is_final_stream_chunk: bool = False):
    ...
    if is_stream_chunk and stream_id:
        ...
        if stream_state and not stream_state["prefix_printed"]:
            ...
            stream_thread_id_hint = stream_state.get("thread_id_for_stream", "N/A")[-6:]
            if stream_thread_id_hint != "N/A": stream_thread_id_hint = stream_thread_id_hint[-6:] # Show last 6 chars
            
            # The prefix itself ensures it starts on new lines relative to cleared input line.
        ...
        # For ALL stream chunks (first or subsequent), just print the chunk.
        ...
        # Print the message, ensuring it ends with a newline.
...
def load_goals_for_console() -> list:
    ...
    if not os.path.exists(GOAL_FILE_CONSOLE): return []
    try:
        with open(GOAL_FILE_CONSOLE, "r", encoding="utf-8") as f:
            ...
            for goal in loaded_goals: # Ensure essential fields for display/sorting
                ...
                goal.setdefault("goal_id", goal.get("goal_id", "N/A_" + str(uuid.uuid4())[:8]))
...
def save_goals_for_console(goals: list) -> None: 
    ...
    _ensure_meta_dir_console()
    # GOAL_FILE_CONSOLE from Section 1
    try:
        with open(GOAL_FILE_CONSOLE, "w", encoding="utf-8") as f:
            json.dump(goals, f, indent=2)
    except Exception as e:
        print_and_restore_prompt(color_text(f"Error saving goals from console: {e}\n{traceback.format_exc()}", Fore.RED))
...
def add_goal_via_console(
    ...
    if approved_by: new_goal_obj["approved_by"] = approved_by
...
def load_changelog_for_console() -> list:
    ...
    _ensure_meta_dir_console()
    # CHANGELOG_FILE_CONSOLE from Section 1
    if not os.path.exists(CHANGELOG_FILE_CONSOLE): return []
    try:
    ...
    try:
        ...
        while True:
            ...
            if user_input_to_process_from_arg:
                ...
                char_typed_bytes = msvcrt.getch() # Read bytes first
                last_activity_time = time.time()
                try:
                    char_typed = char_typed_bytes.decode('utf-8', errors='ignore') # Try UTF-8
                    if not char_typed and len(char_typed_bytes) == 1 : # If decode fails for single byte, try cp1252 (common on Windows)
                        char_typed = char_typed_bytes.decode('cp1252', errors='ignore')

                    if char_typed in ('\r', '\n'): 
                        sys.stdout.write('\n'); sys.stdout.flush()
                        user_input_to_process = partial_input
                        partial_input = "" 
                    elif char_typed_bytes == b'\x08': # Backspace (check bytes)
                        if partial_input:
                            ...
                            sys.stdout.flush()
                    elif char_typed_bytes == b'\x03': # Ctrl+C
                        raise KeyboardInterrupt
                    elif char_typed_bytes == b'\xe0' or char_typed_bytes == b'\x00': # Special keys prefix (arrows, F-keys etc.)
                        msvcrt.getch() # Consume the second byte of the special key sequence
                        # Ignored for now to prevent display issues
                    elif char_typed and char_typed.isprintable(): # Check if it's a printable char
            ...
            if user_input_to_process is not None: 
                ...
                if cmd_lower in {"/exit", "/quit"}:
                    print_and_restore_prompt(color_text("Exiting… Goodbye!", Fore.MAGENTA)); break
                    ...
                    help_parts = [
                        ...
                        f"  {color_text('/addgoal_pri <prio> <text>', Fore.YELLOW):<40} - Add goal with priority (1-Urgent, 4-Low)",
                        f"  {color_text('/addgoal_thread <thread_ID_prefix> <text>', Fore.YELLOW):<40} - Add goal to existing thread",
                        ...
                        f"  {color_text('/reflect', Fore.YELLOW):<40} - Trigger AI reflection cycle on operational prompt",
                        f"  {color_text('/suggestions', Fore.YELLOW):<40} - List pending AI suggestions for new goals",
                        f"  {color_text('/approve <nums_from_suggestions>', Fore.YELLOW):<40} - Approve suggestions to become goals",
                        f"  {color_text('/reject <nums_from_suggestions>', Fore.YELLOW):<40} - Reject suggestions",
                        f"  {color_text('/clearsuggestions', Fore.YELLOW):<40} - Clear all pending suggestions",
                        f"  {color_text('/removegoal <nums_from_active_goals>', Fore.YELLOW):<40} - Remove goal(s) by number from /goals list",
                        f"  {color_text('/recompose <num_from_active_parent_goals>', Fore.YELLOW):<40} - Reset a parent goal for decomposition",
                    ...
                    if VOICE_ENABLED:
                        help_parts.append(f"  {color_text('/voice', Fore.YELLOW):<40} - Activate voice input mode for next command")
                    ...
                    if len(parts) < 3: print_and_restore_prompt(color_text("Usage: /addgoal_thread <thread_id_prefix> <goal_text>", Fore.RED))
                        ...
                        # Search more robustly (active and archived goals)
                        for g_con in reversed(all_console_goals): # Check recent goals first
                    ...
                    # Separate parent goals and subtasks for easier processing
                    parent_goals_display = []
                    sub_goals_map = {} # Store subtasks by their parent_goal_id for quick lookup
                    ...
                    for g_obj_iter in all_goals_list_display:
                        ...
                        if g_obj_iter.get("is_subtask") and g_obj_iter.get("parent_goal_id"):
                            ...
                            if parent_id not in sub_goals_map:
                                sub_goals_map[parent_id] = []
                            ...
                            parent_goals_display.append(g_obj_iter) # Add active, non-subtask goals
                    ...
                    if sorted_active_parent_goals:
                        for idx, parent_g_obj in enumerate(sorted_active_parent_goals, 1):
                            ...
                            # Example of a line:
                            status_str = parent_g_obj.get('status', 'N/A')
                            pri_str = parent_g_obj.get('priority', 'N')
                            thread_hint_str = parent_g_obj.get('thread_id', '----')[-6:]
                            goal_txt_display = parent_g_obj.get('goal','Unnamed Parent Goal')[:70] + ('...' if len(parent_g_obj.get('goal','')) > 70 else '')
                            
                            # Determine color for parent status
                            ...
                            actual_subtask_ids_from_parent = parent_g_obj.get("subtasks") # List of subtask goal_ids
                            # Option 1: Display subtask descriptions if available (from decomposition phase)
                            if status_str == 'decomposed' and parent_g_obj.get('subtask_descriptions') and isinstance(parent_g_obj.get('subtask_descriptions'), list):
                                for sub_idx, sub_desc_text in enumerate(parent_g_obj.get('subtask_descriptions',[]),1):
                                    # Check if this described subtask corresponds to an actual goal object and its status
                                    ...
                                    if sub_goal_obj_for_desc:
                                        sub_stat = sub_goal_obj_for_desc.get('status', 'N/A')
                                        sub_col = col_status_map.get(sub_stat, Fore.WHITE)
                            ...
                            # Option 2: Display actual sub-goal objects linked to this parent
                            elif parent_goal_id_val in sub_goals_map:
                                for sub_idx, sub_g_obj in enumerate(sub_goals_map[parent_goal_id_val], 1):
                                    if sub_g_obj.get("status") in ("completed", "rejected_by_user", "failed_max_retries", "failed_correction_unclear"): # Don't show completed/failed subtasks here unless desired
                                        continue 
                                    sub_status_str = sub_g_obj.get('status', 'N/A')
                                    sub_pri_str = sub_g_obj.get('priority', 'N') # Subtasks inherit priority, but can be shown
                        ...
                        output_parts.append(color_text("  (No active non-subtask goals.)", Fore.GREEN))
                    
                    # Display Archived Goals (as before, or a more compact version)
                    archived_display_goals = sorted(
                        [g for g in all_goals_list_display if isinstance(g,dict) and g.get("status") in ("completed", "rejected_by_user", "failed_max_retries", "failed_correction_unclear") and not g.get("is_subtask")], # only show archived parents for brevity
                         key=lambda g_item: (g_item.get("history", [{}])[-1].get("timestamp", g_item.get("created_at", "1970-01-01T00:00:00Z"))), reverse=True # Sort by last history entry
                    )
                    ...
                    if archived_display_goals:
                         for idx_arc, g_arc_obj in enumerate(archived_display_goals[:5], 1):
                            status_arc_str = g_arc_obj.get('status', 'N/A')
                            goal_arc_txt_display = g_arc_obj.get('goal','Unnamed Goal')[:70] + ('...' if len(g_arc_obj.get('goal','')) > 70 else '')
                    ...
                    print_and_restore_prompt(color_text("\nCurrent Full System Prompt:\n", Fore.CYAN) + load_prompt()) # Uses load_prompt from Section 1
                
                elif cmd_lower == "/update_prompt":
                    ...
                    auto_update_prompt() # Logs its own success/failure
                    ...
                    # This command ensures the mission-driven directive is active as a high-priority goal.
                    try:
                        import mission_manager # Ensure it's loadable here too
                    ...
                    if os.path.exists(ACTIVE_GOAL_FILE_CONSOLE):
                        try:
                            with open(ACTIVE_GOAL_FILE_CONSOLE, "r", encoding='utf-8') as f_active: 
                                content_active = f_active.read()
                    ...
                    status_msg_parts.append(f"{color_text(f'Pending Suggestions ({len(pending_suggs_status)}):', Fore.YELLOW)} " + 
                                         (", ".join([str(s['suggestion'])[:30]+"..." for s in pending_suggs_status[:2]]) if pending_suggs_status else "(None)"))
                    
                    # LAST_REFLECTION_CONSOLE_FILE from Section 1
                    last_reflect_ts_val = None 
                    if os.path.exists(LAST_REFLECTION_CONSOLE_FILE): # This is prompt_manager's timestamp file
                        try:
                    ...
                    status_counts_stats, priority_counts_pending_stats = {}, {}
                    active_threads_stats = set()
                    ...
                    try: # TOOL_REGISTRY_CONSOLE_FILE from Section 1
                        if os.path.exists(TOOL_REGISTRY_CONSOLE_FILE):
                            with open(TOOL_REGISTRY_CONSOLE_FILE,'r',encoding='utf-8') as f_tr_stat: 
                                content_tr_stat = f_tr_stat.read()
                            ...
                            if isinstance(tool_reg_stats_data, list): # Ensure it's a list
                                ...
                                unstable_tools_list_stat = [t.get('name') for t in tool_reg_stats_data if isinstance(t,dict) and t.get('total_runs',0) > 2 and (t.get('failure_count',0) / t['total_runs']) > 0.6] # Stricter criteria
                    ...
                    for i_sugg_disp, s_obj_disp in enumerate(pending_suggs_disp, 1):
                        sugg_text_disp = str(s_obj_disp.get('suggestion','N/A')).split("\n\nNone")[0].split("\n\n**None**")[0].strip() # Clean common LLM artifacts
                        sugg_text_preview = sugg_text_disp[:100] + ('...' if len(sugg_text_disp) > 100 else '')
                        ts_sugg_disp = format_timestamp_console(s_obj_disp.get('timestamp', 'N/A')) # Helper from Sec 2
                        ...
                        if sugg_thread_hint: sugg_out_str += f" (Rel.Th: ..{sugg_thread_hint[-6:]})"
                        sugg_out_str += "\n"
                    ...
                    if len(parts_sugg_act) < 2: sugg_act_msg_str = color_text(f"Usage: /{cmd_name_sugg_act} <nums_from_/suggestions>\n", Fore.RED)
                        ...
                        processed_sugg_timestamps_for_this_command = set() # To avoid double processing if numbers overlap due to list changes

                        for idx_0b_s_act in sorted(indices_sugg_0b_act, reverse=True): # Process from highest index to avoid issues with list modification
                            if 0 <= idx_0b_s_act < len(pending_suggs_to_act_on):
                                ...
                                if sugg_timestamp_id in processed_sugg_timestamps_for_this_command: continue # Already handled

                                original_sugg_in_main_list_found = False
                                for i_orig_sugg_main, orig_sugg_main_obj in enumerate(all_suggs_to_act_on):
                                    if isinstance(orig_sugg_main_obj, dict) and orig_sugg_main_obj.get('timestamp') == sugg_timestamp_id and \
                                       orig_sugg_main_obj.get('suggestion') == sugg_to_proc_obj.get('suggestion') and \
                                       orig_sugg_main_obj.get("status","pending") == "pending":
                                        ...
                                        if is_approve_cmd:
                                            ...
                                            sugg_related_thread_id = sugg_to_proc_obj.get('related_thread_id') # From suggestion engine
                                            # If no related_thread_id on suggestion, add to a new thread
                                            if not sugg_related_thread_id: sugg_related_thread_id = str(uuid.uuid4())
                                            
                                            add_goal_via_console(sugg_text_cleaned, source="suggestion_approved", approved_by="user_console", 
                                                               priority=CONSOLE_PRIORITY_NORMAL, thread_id_to_relate=sugg_related_thread_id) # Helper from Sec 2
                                            sugg_act_msg_str += color_text(f"✅ Approved & added as goal: '{sugg_preview_for_log}' (Thread: ..{sugg_related_thread_id[-6:]})\n", Fore.GREEN)
                                        else: # Reject
                                        ...
                                        action_taken_on_sugg = True
                                        original_sugg_in_main_list_found = True
                                        processed_sugg_timestamps_for_this_command.add(sugg_timestamp_id)
                                        break # Found and processed this specific suggestion from pending list in main list
                                
                                if not original_sugg_in_main_list_found: 
                                    sugg_act_msg_str += color_text(f"Error: Suggestion (idx {idx_0b_s_act+1} from pending list) not found or already processed in main suggestion list.\n", Fore.RED)
                            else: 
                                sugg_act_msg_str += color_text(f"⚠️ Invalid suggestion number: {idx_0b_s_act + 1}. Use numbers from /suggestions list.\n", Fore.RED)
                        
                        if action_taken_on_sugg: save_suggestions_console(all_suggs_to_act_on) # Func from Sec 1
                        elif not sugg_act_msg_str: # No valid indices and no previous error message
                             sugg_act_msg_str = color_text(f"No action taken. Ensure numbers correspond to current /suggestions list.", Fore.YELLOW)
                    ...
                    # Keep non-pending, remove pending
                    ...
                    if cleared_count > 0:
                        ...
                        print_and_restore_prompt(color_text(f"🗑️ Cleared {cleared_count} pending suggestion(s). Non-pending suggestions retained.", Fore.YELLOW))
                    else:
                        print_and_restore_prompt(color_text("No pending suggestions to clear.", Fore.YELLOW))
                    ...
                    remove_msg_str = ""
                    if len(parts_rem_cmd) < 2: 
                        remove_msg_str = color_text("Usage: /removegoal <nums_from_active_goals_in_/goals>\nNumbers refer to the main (non-subtask) goals listed.", Fore.RED)
                    else:
                        indices_rem_0b_cmd = parse_indices_from_command(parts_rem_cmd[1])
                        all_goals_state_rem_cmd = load_goals_for_console()
                        
                        # Replicate the "active parent goals" list creation and sorting from /goals command
                        ...
                        # Check if executor and its scoring function are available for sorting
                        ...
                        goal_ids_to_rem_set_cmd = set()
                        valid_indices_provided = True
                        if not indices_rem_0b_cmd and parts_rem_cmd[1].strip(): 
                            remove_msg_str += color_text("No valid goal numbers provided for removal.\n", Fore.YELLOW)
                            valid_indices_provided = False
                        ...
                        for idx_0b_r_cmd in indices_rem_0b_cmd:
                            if 0 <= idx_0b_r_cmd < len(active_goals_for_rem_cmd): 
                                ...
                                goal_ids_to_rem_set_cmd.add(goal_to_remove_obj["goal_id"]) # type: ignore
                                # Also mark any of its subtasks for removal
                                if isinstance(goal_to_remove_obj.get("subtasks"), list): # type: ignore
                                    for sub_id in goal_to_remove_obj.get("subtasks", []): # type: ignore
                                        goal_ids_to_rem_set_cmd.add(sub_id)
                                remove_msg_str += color_text(f"Marked goal '{str(goal_to_remove_obj.get('goal',''))[:30]}...' (and its subtasks) for removal.\n", Fore.CYAN)
                            else: 
                                remove_msg_str += color_text(f"⚠️ Invalid goal number '{idx_0b_r_cmd + 1}'. Use numbers from main goals in /goals list.\n", Fore.RED)
                                valid_indices_provided = False # Mark that at least one index was bad
                        ...
                        if valid_indices_provided and goal_ids_to_rem_set_cmd:
                            ...
                            if removed_count_cmd > 0:
                                save_goals_for_console(final_kept_goals_cmd)
                                remove_msg_str += color_text(f"🗑️ Removed {removed_count_cmd} goal object(s) (including subtasks of selected parents).\n", Fore.YELLOW)
                            else: # This case should ideally not be hit if valid_indices_provided was true and goal_ids_to_rem_set_cmd was populated
                                remove_msg_str += color_text("No goals were ultimately removed. They might have already been processed or numbers were invalid.\n", Fore.YELLOW)
                        elif valid_indices_provided and not goal_ids_to_rem_set_cmd and parts_rem_cmd[1].strip(): 
                            remove_msg_str += color_text("No matching active parent goals found for the provided numbers.\n", Fore.YELLOW)
                        elif not valid_indices_provided and not goal_ids_to_rem_set_cmd : # No valid numbers were parsed at all
                             remove_msg_str = color_text("No valid goal numbers entered for removal.\n", Fore.YELLOW)


                    print_and_restore_prompt(remove_msg_str.strip() if remove_msg_str else "No removal action taken.")
                    ...
                    recomp_msg_str = ""
                    if len(parts_recomp_cmd) < 2 or not parts_recomp_cmd[1].strip().isdigit(): 
                        recomp_msg_str = color_text("Usage: /recompose <num_from_active_parent_goals_in_/goals>\nOnly parent (non-subtask) goals can be recomposed.", Fore.RED)
                    else:
                        ...
                        if 0 < idx_1b_recomp_cmd <= len(sorted_active_parents_recomp):
                            ...
                            if goal_to_recomp_obj.get("is_subtask"): # Should not happen due to filter above, but double check
                                recomp_msg_str = color_text(f"⚠️ Goal '{goal_text_recomp_val}...' is a subtask and cannot be recomposed directly. Recompose its parent.", Fore.YELLOW)
                            else:
                                modified_list_recomp_cmd = []
                                found_modified_recomp_cmd = False
                                subtasks_of_recomposed_parent_removed_count = 0
                                for g_main_iter_cmd in all_goals_recomp_cmd:
                                    ...
                                    if g_main_iter_cmd.get("goal_id") == goal_id_recomp_val:
                                        g_main_iter_cmd["status"] = "pending" # Reset status
                                        g_main_iter_cmd.pop("subtasks", None) # Clear subtask ID list
                                        g_main_iter_cmd.pop("subtask_descriptions", None) # Clear descriptions
                                        # Clear execution artifacts and evaluation
                                        for key_pop_recomp in ["review", "tool_file", "execution_result", "error", "failure_category", "evaluation", "used_existing_tool"]: 
                                            g_main_iter_cmd.pop(key_pop_recomp, None)
                                        g_main_iter_cmd.setdefault("history",[]).append({
                                            "timestamp":datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"), 
                                            "status":"pending", 
                                            "action":"Recomposed by user for new decomposition round."
                                        })
                                        g_main_iter_cmd["updated_at"] = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
                                        modified_list_recomp_cmd.append(g_main_iter_cmd)
                                        found_modified_recomp_cmd = True
                                    elif g_main_iter_cmd.get("parent_goal_id") == goal_id_recomp_val: 
                                        # This subtask belongs to the parent being recomposed; remove it.
                                        subtasks_of_recomposed_parent_removed_count += 1
                                        # Do not append to modified_list_recomp_cmd
                                    else: 
                                        modified_list_recomp_cmd.append(g_main_iter_cmd)
                                ...
                                if found_modified_recomp_cmd:
                                    ...
                                    if subtasks_of_recomposed_parent_removed_count > 0: 
                                        recomp_msg_str += f"{subtasks_of_recomposed_parent_removed_count} previous subtask object(s) removed.\n"
                                    else: recomp_msg_str += "No existing subtask objects found/removed for this parent.\n"
                                else: 
                                    # This case should be rare if indexing is correct
                                    recomp_msg_str = color_text(f"⚠️ Could not find GID {str(goal_id_recomp_val)[-6:]} to modify.", Fore.RED)
                        else: 
                            recomp_msg_str = color_text(f"⚠️ Invalid goal number '{idx_1b_recomp_cmd}'. Use a number from the active, non-subtask (parent) goals shown in /goals.\n", Fore.RED)
                    ...
                    for entry_cl in reversed(cl_data_disp[-10:]): # Show last 10
                ...
                elif cmd_lower == "/voice_cancel": # Manual voice cancel if stuck
